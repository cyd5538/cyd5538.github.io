...
운영체제에 대한 이해를 위해서는 먼저 컴퓨터 시스템의 구성을 알아야 합니다. 컴퓨터 시스템은 주로 하드웨어와 소프트웨어로 구성되어 있습니다.

- **하드웨어**: CPU, 메모리, 저장장치, 입출력장치, 네트워크 장비 등
- **소프트웨어**: 응용소프트웨어(웹브라우저, PDF viewer, 파워포인트)와 시스템 소프트웨어(운영체제)

운영체제는 주요한 소프트웨어로, 컴퓨터 시스템의 자원을 효율적으로 관리하고 프로그램이 원활하게 동작할 수 있도록 서비스를 제공합니다.
...
- **컴퓨터 시스템 관리**: 자원(하드웨어 및 소프트웨어 자원, 데이터)를 효율적으로 관리하여 시스템이 원활하게 동작하도록 함
- **사용자 지원**: 사용자 명령을 해석하고 실행하여 하드웨어와 소프트웨어 사이의 매개체 역할을 하며 사용자에게 편의성을 제공함

### 운영체제가 없던 초기의 컴퓨터 시스템
- 응용 프로그램이 직접 컴퓨터 시스템 의자원 제어
  - 응용 프로그램 개발자는 하드웨어 제어방법을 잘 알아야 함.
- 여러 사용자가 하드웨어를 공유하는 경우 자원 분한 어려움
...
- 크게 일체형 커널과 마이크로 커널로 나뉨

**일체형 커널(monolithic kernal)**
...
- 메모리 관리, 멀티태스킹, 프로세스 간 통신(IPC) 등 최소한의 요소만 커널에 남김
- 장점
...
- 사용자 모드에서 응용 프로그램이 동작하다가 시스템 호출이 발생하면 그 순간 모드가 바뀐다.
- 커널 모드와 사용자 모드가 계속 변화되면서 일을 처리함.
...
- 컴퓨터 시스템의 모든 장치 관리
- 시스템 장치의 할당, 작동, 반환 ex) 하드 디스크, SSD, 키보드 마우스, 프린터 등
...
- 사람(오퍼레이터) 하던 일을 프로그램이 빠르게 처리하면서 전체적인 작업 처리 속도가 향상됨
- 나중에 들어온 작업은 앞선 작업들이 모두 끝날 떄까지 아무런 상호작용 없이 기다려야만 함
...
- 분산 시스템을 관리하기 위한 운영체제
    - 분산 시스템: 2개 이상의 컴퓨터가 시스템이 네트워크로 서로 연결되어 서로의 자원을 이용하는 시스템
